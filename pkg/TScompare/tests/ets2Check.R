...
# consider changing con1 to wfs
#wfs or (older?) wfsv Meta is not properly constructed
# temporaily use duplicate etsv for testing
   con1 <- {if ("" != user1)  
   ...
   con2 <-  {if ("" != user1) 
            try(TSconnect("MySQL", dbname="etsv", username=user, password=passwd, host=host1)) 
       else try(TSconnect("MySQL", dbname="etsv")) 
      }

   if (!inherits(con1, "try-error") & !inherits(con2, "try-error")) {
      #ids <- AllIds(con1, vintage=NULL) #with etsv takes several minutes 1284552
      #ids <- AllIds(con1, vintage="2008-06-13")#with etsv takes seconds  823595
      ids <- AllIds(con1, vintage="current")   #with etsv takes seconds  616932
      #length(ids)
      #ids <- cbind(ids, ids)
   ...
   # nice to do all, but too slow for regular build
   #  ids <- ids[1:100]
   #quick test
