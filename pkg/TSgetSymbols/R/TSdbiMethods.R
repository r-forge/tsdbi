...
setClass("getSymbolsDriver", representation("DBIDriver", Id = "character")) 

getSymbols <- function() {
  drv <- "getSymbols"
  attr(drv, "package") <- "TSgetSymbols"
  new("getSymbolsDriver", Id = drv)
  }

# require("DBI") for this
setClass("TSgetSymbolsConnection", contains=c("DBIConnection", "conType","TSdb"),
...
setMethod("dbDisconnect", signature(conn="TSgetSymbolsConnection"), 
     definition=function(conn,...) TRUE)
#######     end kludges   ######

setMethod("TSconnect",   signature(drv="getSymbolsDriver", dbname="character"),
  definition= function(drv, dbname, user="", password="", host="", ...){
   #  user / password / host  for future consideration
   if (is.null(dbname)) stop("dbname must be specified")
   if (dbname == "FRED") {
      #there could be a better test
      con <- try(getSymbols('CPIAUCNS',src='FRED'), silent = TRUE)
      if(inherits(con, "try-error")) 
         stop("Could not establish TSgetSymbolsConnection to ",  dbname)
      close(con)
      }
   else if (dbname == "yahoo") {
      con <- try(getSymbols(c('QQQQ',src='yahoo'), silent = TRUE)
      if(inherits(con, "try-error")) 
         stop("Could not establish TSgetSymbolsConnection to ",  dbname)
      close(con)
      }
   ...
   new("TSgetSymbolsConnection", drv="getSymbols", dbname=dbname, hasVintages=FALSE, hasPanels=FALSE,
    	  user = user, password = password, host = host ) 
   } )


setMethod("TSdates",
  signature(serIDs="character", con="TSgetSymbolsConnection", vintage="ANY", panel="ANY"),
...
setMethod("TSget",     signature(serIDs="character", con="TSgetSymbolsConnection"),
   definition= function(serIDs, con, TSrepresentation=options()$TSrepresentation,
    ...
    for (i in seq(length(serIDs))) {
       ...
       for (rpt in seq(repeat.try)) {
           r <- try(do.call("getSymbols", argsi))
...
#setMethod("TSput",     signature(x="ANY", serIDs="character", con="TSgetSymbolsConnection"),
#   definition= function(x, serIDs=seriesNames(data), con, ...)   
#    "TSput for TSgetSymbols connection not supported." )

setMethod("TSdescription",   signature(x="character", con="TSgetSymbolsConnection"),
   definition= function(x, con=getOption("TSconnection"), ...)
        "TSdescription for TSgetSymbols connection not supported." )


setMethod("TSdoc",   signature(x="character", con="TSgetSymbolsConnection"),
   definition= function(x, con=getOption("TSconnection"), ...)
        "TSdoc for TSgetSymbols connection not supported." )

setMethod("TSlabel",   signature(x="character", con="TSgetSymbolsConnection"),
   definition= function(x, con=getOption("TSconnection"), ...)
        "TSlabel for TSgetSymbols connection not supported." )
